...
router.get('/:module_api_name/:record_id/photo',
  ...
  /**
 * Get Related Records Attachments
 * https://www.zoho.com/crm/developer/docs/api/v2/get-attachments.html
 * 
 * Response
 * https://www.zoho.com/crm/developer/docs/api/v2/get-attachments.html
 */
router.get('/:module_api_name/:record_id/Attachments/:id_attachments',
middlewares.accessTokenMiddleware,
async function (request, response, next) {
  ...
  axiosSingleton.get(`${ZOHO_API_CRM_RESOURCE}/${module_api_name}/${record_id}/Attachments/${id_attachments}`, {
    ...
    .then(function (recordsResponse) {
      debug('get', '/:module_api_name/:record_id/Attachments/:id_attachments', true);
      response.set('Content-Type', 'image/png');
      response.send(recordsResponse.data).end();
    })
    .catch(function (error) {
      debug('get', '/:module_api_name/:record_id/Attachments/:id_attachments', 'error', error);
      next(new Error(error));
    });
});
